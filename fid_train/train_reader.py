...
def train(model, optimizer, scheduler, step, train_dataset, eval_dataset, opt, collator, best_dev_em, checkpoint_path):
    ...
    epoch = 1
    model.train()
    progress_bar = tqdm(range(opt.total_steps))
    while step < opt.total_steps:
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            print(context_ids)
            print(tokenizer.batch_decode(context_ids[0]))
            labels_ = labels.clone().contiguous()
            labels_[labels_[:, :] == -100] = tokenizer.pad_token_id
            print(tokenizer.batch_decode(labels_))
            train_loss = model(
